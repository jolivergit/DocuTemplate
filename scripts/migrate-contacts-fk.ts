 ...
 *
 * Idempotent: safely re-runnable after columns have already been dropped.
 */
...
/** Returns true if the given column still exists on the table. */
...
async function main() {
  ...
  // Step 1: Backfill contact_id from legacy inline fields — only if those columns still exist.
  const hasInlineColumns = await columnExists("lead_companies", "contact_full_name");

  if (!hasInlineColumns) {
    console.log("Inline contact columns already dropped — skipping backfill step.");
  } else {
    ...
    if (orphanRows.length > 0) {
      ...
      for (const row of orphanRows) {
        ...
        if (!contactId && row.contact_full_name) {
          const { rows: existing } = await pool.query<{ id: string }>(
            `SELECT id FROM contacts WHERE user_id = $1 AND LOWER(full_name) = LOWER($2) LIMIT 1`,
            [userId, row.contact_full_name]
          );
          if (existing.length > 0) {
            contactId = existing[0].id;
          }
  ...
  // Step 2: Drop the four inline columns (IF EXISTS makes each drop safe to re-run).
  console.log("Dropping inline columns from lead_companies (if present)...");
  await pool.query(`
